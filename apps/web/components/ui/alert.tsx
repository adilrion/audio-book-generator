  ...
  'relative grid w-full grid-cols-[0_1fr] items-start gap-y-0.5 rounded-lg border px-4 py-3 text-sm has-[>svg]:grid-cols-[calc(var(--spacing)*4)_1fr] has-[>svg]:gap-x-3 [&>svg]:size-4 [&>svg]:translate-y-0.5 [&>svg]:text-current',
  {
    variants: {
      variant: {
        ...
        destructive: 'border-destructive/30 bg-destructive/[0.06] text-destructive dark:bg-destructive/10 [&>svg]:text-current *:data-[slot=alert-description]:text-destructive/90',
        warning: 'border-warning/40 bg-warning/[0.08] text-foreground dark:bg-warning/10 [&>svg]:text-warning',
        success: 'border-success/30 bg-success/[0.07] text-foreground [&>svg]:text-success',
        info: 'border-info/30 bg-info/[0.06] text-foreground [&>svg]:text-info',
      },
    },
